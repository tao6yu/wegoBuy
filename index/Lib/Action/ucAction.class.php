...
class ucAction extends baseAction{
   ...
   function account_message(){
	...
	function tao_login(){
        ...
        $redirect_uri = $this->site_root . "index.php?m=uc&a=tao_" . $type.'&state=1';

        $login_url = "https://oauth.taobao.com/authorize?response_type=code&client_id={$this->setting['taobao_appkey']}&redirect_uri=$redirect_uri";
        header("Location:$login_url");
	}
	function tao_callback(){
		 $code = $_REQUEST['code'];   //通过访问https://oauth.taobao.com/authorize获取code
		 $grant_type = 'authorization_code';
		 $redirect_uri = $this->site_root . "index.php?m=uc&a=tao_callback";  //此处回调url要和后台设置的回调url相同
		...
		if($is_new){			
			$tao_info=array('user_info'=>$encode_token);
			$data=array(
					'name'=>$name,
					//'img'=>$res->figureurl_2,
					'last_time'=>time(),
...
//	        }			
	...
	function login(){
        ...
        if (isset($_POST['dosubmit']) || $this->isAjax()){
            ...
            $user=$this->user_mod->where("name='".$name."' and passwd='".md5($passwd)."'")->find();
